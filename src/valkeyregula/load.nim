...
type InsertFn* = proc(fact_type: string, fields: Table[string, string]): Result[void, BridgeError] {.raises: [].}
proc recover*(scan_fn: ValkeyScanFn, get_fn: ValkeyGetFn, insert_fn: InsertFn,
              prefix: string): Result[int, BridgeError] =
  let keys = scan_fn(prefix)
  if keys.is_bad: return Result[int, BridgeError].bad(keys.err)
  var count = 0
  for key in keys.val:
